    ...
    if (HCE) {
	    ...
	    hitCount = TubeHits->entries();
            for (G4int h=0; h<hitCount; h++) {
                ...
                Tube_outFile << event_number << "," << trID << "," << parentID << "," << name << "," << proc <<"," << x <<","<<y<<","<<z<<","<<
        ...
        // for (int i=0; i < PMT_index.size();i++){
        //     std::cout << "PMT eindex" << std::endl;
        //     std::cout << PMT_index[i] << ","<< PMT_x[i] << "," << PMT_y[i] << "," << PMT_z[i]<< std::endl;
        // }
        ...
        for (int i = 0; i < PMT_index_reduced.size();i++){
            ...
            PMT_outFile << event_number << "," << PMT_index_reduced[i] << "," << x_pmt << "," << y_pmt << "," << z_pmt << "," << count <<G4endl;
        }
